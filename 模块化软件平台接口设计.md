...
- void InitPIRegu(PI_REGU *p, float alpha, float beta, float gamma, float max, float min)
  - 功能：初始化PI控制器
  - 返回值：无
  - 参数：p: PI控制器结构体指针，alpha: PI控制器参数，beta：PI控制器参数， gamma: PI控制器参数，max: PI控制器输出最大值，min：PI控制器输出最小值。
  - 说明：
- PISetPara(PI_REGU *p, float alpha, float beta， float gamma)
  - 功能： 设置PI控制器控制参数
  - 返回值： 无
  - 参数： p: PI控制器结构体指针，alpha: PI控制器参数，beta：PI控制器参数, gamma: PI控制器参数，
  - 说明：
...
- uint16_t ResetSwTimer(struct SW_TIMER Timer);
...
- void InitAlarm(struct ST_ALARM* Alarm, uint16_t Premise, uint16_t Attr, int16_t Thres, int16_t ThresRecover);
  - 功能：初始化一个报警模块。
  - 返回值：无
  - 参数：Alarm: 报警模块结构体指针， Premise: 报警前提条件（状态），Attr: 报警属性，Thres：报警限值，ThresRecover: 报警恢复限值。
- uint16_t UpdateAlarm(struct ST_ALARM*  alarm, uint16_t input);
- void EnableAlarm(struct ST_ALARM*  alarm);
- void DisableAlarm(struct ST_ALARM*  alarm);
...
这样所有的故障在程序中只需要两个uint16_t型来表示，利于程序中对于故障状态的判断和处理。在向UI发送数据时也不用一个故障就发送一个字，大大减少了数据传输量。
